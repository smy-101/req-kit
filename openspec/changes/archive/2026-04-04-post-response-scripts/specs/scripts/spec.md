...
系统 SHALL 同时支持后置脚本执行。`ScriptService` SHALL 提供 `executePostScript()` 方法，沙箱中用 `response` 对象替代 `request` 对象，并额外提供 `tests` 断言收集对象。
...
系统 SHALL 收集脚本中 `console.log()` 的输出，并在代理响应中附带 `script_logs` 字段（前置脚本日志）和 `post_script_logs` 字段（后置脚本日志）。
...
`ScriptResult` SHALL 包含 `variables` 字段（`Record<string, string>`），包含脚本通过 `variables.set()` 设置的所有变量。

代理响应 SHALL 包含 `script_variables` 字段（前置脚本变量）和 `post_script_variables` 字段（后置脚本变量），值与各自 `ScriptResult.variables` 一致。

前端接收到 `script_variables` 和 `post_script_variables` 后 SHALL 将其依次合并到 `store.runtimeVars` 中。
