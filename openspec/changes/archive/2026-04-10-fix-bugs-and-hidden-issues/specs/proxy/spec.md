...
系统 SHALL 在 `injectAuth()` 中对 `auth_config` 的 JSON.parse 调用添加异常处理。当 `auth_config` 为字符串但 JSON 解析失败时，系统 SHALL 输出 `console.warn` 日志并原样返回 headers/params（不注入认证信息），不中断请求管道。

系统 SHALL 在 `recordHistory()` 的 catch 块中输出 `console.error` 日志，记录历史写入失败的错误信息，而非静默忽略。
