...
test.describe('历史记录加载验证', () => {
  test('点击历史记录加载 POST 请求并验证数据', async ({ page }) => {
    ...
    const testUrl = `https://httpbin.org/post?h=${uniqueId}`;
    ...
    const targetItem = page.locator('.history-item').filter({ hasText: `h=${uniqueId}` }).first();
    ...
    await expect(page.locator('#url-input')).toHaveValue(new RegExp(`h=${uniqueId}`), { timeout: 10000 });
  ...
  test('匹配的历史记录在当前标签页加载不创建新标签', async ({ page }) => {
    ...
    // 发送 GET 请求到特定 URL
    const testUrl = `https://httpbin.org/get?unique=${Date.now()}`;
    await page.locator('#url-input').fill(testUrl);
    await page.waitForTimeout(400);
    await page.locator('#send-btn').click();
    await expect(page.locator('#response-status')).toContainText('200', { timeout: 15000 });
